...
def footer_for(cfg):
    f = FOOTER
    # Drop the chrome's existing link to the old, superseded /sealcoating/
    # page first (this reconciliation replaces it), then insert ours in the
    # normal spot so there's exactly one Seal Coating entry, not two.
    f = f.replace('\n          <li><a href="/sealcoating/">Sealcoating</a></li>', '', 1)
    services_old = '<li><a href="/concrete-cleaning-tipp-city-ohio/">Concrete Cleaning</a></li>'
    services_new = services_old + '\n          <li><a href="/seal-coating-tipp-city-ohio/">Seal Coating</a></li>'
    assert services_old in f, "chrome source's footer services list changed shape"
...
def mobile_drawer_and_after_head(cfg):
    ...
    assert old in h, "chrome source's mobile drawer changed shape"
